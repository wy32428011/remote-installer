...
if ([string]::IsNullOrWhiteSpace($Username) -and -not [string]::IsNullOrWhiteSpace($envUsername) -and -not [string]::IsNullOrWhiteSpace($envPassword)) {
    $Username = $envUsername
}
if ([string]::IsNullOrWhiteSpace($Password) -and -not [string]::IsNullOrWhiteSpace($envPassword)) {
...
# 10.1 配置用户权限（服务就绪后）
Write-Progress "ConfiguringUsers" 88
Write-Color "`n10.1 配置用户权限:" "Yellow"

if (Test-Path $rabbitmqctl) {
    $userExists = $false
    try {
        $usersOutput = & $rabbitmqctl list_users 2>&1
        if ($usersOutput -match "^\s*$([Regex]::Escape($Username))\s") {
            $userExists = $true
        }
    } catch {
        Write-Color "读取用户列表失败，继续尝试创建用户" "Yellow"
    }

    if ($userExists) {
        Write-Color "用户 $Username 已存在，更新密码..." "Yellow"
        & $rabbitmqctl change_password $Username $Password 2>&1 | Out-Null
    } else {
        Write-Color "创建用户 $Username ..." "Yellow"
        & $rabbitmqctl add_user $Username $Password 2>&1 | Out-Null
    }

    Write-Color "设置 $Username 为管理员..." "Yellow"
    & $rabbitmqctl set_user_tags $Username administrator 2>&1 | Out-Null
    Write-Color "设置 $Username 用户权限..." "Yellow"
    & $rabbitmqctl set_permissions -p / $Username ".*" ".*" ".*" 2>&1 | Out-Null
    Write-Color "用户 $Username 已配置为管理员并授予权限" "Green"
}
...
Write-Color "默认用户：$Username" "Yellow"
Write-Color "默认密码：$Password" "Yellow"
Write-Color "注意：已为用户 $Username 配置管理权限" "Yellow"
