...
- CentOS 7 离线目录还需要包含 `libwebsockets` 与 `libuv` RPM；当前仓库使用 `libwebsockets-3.0.1-2.el7.x86_64.rpm`，已验证提供 `libwebsockets.so.13()(64bit)`。
...
## MariaDB Offline Package Layout

MariaDB 按系统维护离线资源目录，推荐放置在以下路径：

- `RemoteInstaller/Scripts/MariaDB/mariadb-centos7`
- `RemoteInstaller/Scripts/MariaDB/mariadb-ubuntu/24`

### Linux (CentOS 7)

- 仅支持目录型离线资源。
- 根目录至少包含 `MariaDB-server-*.rpm` 或 `mariadb-server-*.rpm`。
- CentOS 7 会执行严格离线能力校验和 `yum --disablerepo='*' --setopt=tsflags=test localinstall` 事务预检，避免运行时才暴露缺失依赖。
- 版本化 RPM 依赖会先按能力名检查本地 RPM 是否提供，再交由 yum 事务预检确认真实版本约束，避免 `>=`、`=` 等表达式导致误报。
- 当前仓库已补齐 `galera-4`、`Judy`、`libzstd`、`unixODBC`、`lsof`、`rsync`、`pv`、`python3`、`java-1.8.0-openjdk-headless`、Perl 模块、NSS/NSPR 与 Java/Python 传递依赖。
- `MariaDB-test-*.rpm` 是上游测试组件，不是服务运行必需包；安装脚本会跳过该 RPM，避免测试包损坏影响服务部署。

### Linux (Ubuntu 24)

- 仅支持目录型离线资源，目录内必须包含 `Packages` 与 `Release` 元数据。
- 本地 APT 源使用 `deb [trusted=yes] file://... ./`，不再依赖 MariaDB 旧签名文件；`InRelease` 与 `Release.gpg` 不作为离线安装条件。
- 当前仓库已补齐 `libconfig-inifiles-perl`、`libdbi-perl`、`lsof`、`rsync`，并针对本轮 Ubuntu 24.10 测试机补入 `libncurses6`、`libtinfo6`。
- 增删 DEB 后必须重建 `Packages` 与 `Release`，确保 APT 能识别混合离线包集合。
