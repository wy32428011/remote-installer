...
Write-Host "JDK 11 Windows 安装脚本开始..."
...
if (Test-Path $PackagePath -PathType Container) {
    ...
    switch ($extension) {
        ...
        default {
            Write-Error "仅支持目录、zip、msi、exe 格式的 JDK 11 Windows 离线资源"
            exit 1
        }
    }
}
...
[Environment]::SetEnvironmentVariable("JDK11_HOME", $resolvedInstallPath, "Machine")
...
if ($versionLine -notmatch '11\.') {
    Write-Error "检测到的版本不是 JDK 11：$versionLine"
    exit 1
}
