...
if [[ "$PACKAGE_PATH" == *.tar.gz ]] || [[ "$PACKAGE_PATH" == *.tar ]]; then
    ...
    # 安装 deb 包，显示所有输出以便调试
    echo "正在安装 DEB 包..."
    DEBIAN_FRONTEND=noninteractive dpkg -i "$PACKAGE_PATH" 2>&1 || \
...
# 辅助函数：删除配置项（含注释/空白/冒号前空格）
remove_config_key() {
    local key="$1"
    local escaped_key
    escaped_key=$(echo "$key" | sed 's/\./\\./g')
    sed -i -E "/^[[:space:]]*#?[[:space:]]*${escaped_key}[[:space:]]*:/d" "$CONFIG_FILE" 2>/dev/null || true
}
...
remove_config_block() {
    ...
    BEGIN { skip=0 }
    {
        if ($0 ~ "^[[:space:]]*#?[[:space:]]*" k "[[:space:]]*:") { skip=1; next }
        if (skip==1) {
