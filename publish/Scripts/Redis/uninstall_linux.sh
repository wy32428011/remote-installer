...
if [ "$OS" = "Debian" ]; then
    ...
    if [ -n "$INSTALLED_PKGS" ]; then
        ...
        if [ "$KEEP_DATA" = true ]; then
            DEBIAN_FRONTEND=noninteractive apt-get remove -y $INSTALLED_PKGS 2>/dev/null || true
        else
            DEBIAN_FRONTEND=noninteractive apt-get purge -y $INSTALLED_PKGS 2>/dev/null || true
        fi
...
if command -v ss &> /dev/null; then
    if ss -tuln 2>/dev/null | grep -q ':6379[[:space:]]'; then
        echo "警告：端口 6379 仍在监听"
        FAILED=1
    else
        echo "端口 6379：已释放"
    fi
elif command -v netstat &> /dev/null; then
    if netstat -tuln 2>/dev/null | grep -q ':6379[[:space:]]'; then
        echo "警告：端口 6379 仍在监听"
        FAILED=1
    else
        echo "端口 6379：已释放"
